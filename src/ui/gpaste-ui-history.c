                 ...
                 gpointer      user_data)
{
    ...
    guint64 search_results_size = 0;

    if (priv->search_results)
    {
        /* A failed search leaves this NULL, and g_strv_length() has no more
         * patience with that than the daemon side had (c0022cf2). */
        search_results_size = g_strv_length (priv->search_results);
