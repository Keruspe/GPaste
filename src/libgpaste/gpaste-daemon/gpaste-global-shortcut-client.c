...
#define G_PASTE_PORTAL_REQUEST_INTERFACE_NAME "org.freedesktop.portal.Request"
#define G_PASTE_PORTAL_SESSION_INTERFACE_NAME "org.freedesktop.portal.Session"

#define G_PASTE_PORTAL_REQUEST_OBJECT_PATH G_PASTE_GLOBAL_SHORTCUT_OBJECT_PATH "/request"

#define G_PASTE_PORTAL_SIG_RESPONSE "Response"
#define G_PASTE_PORTAL_CLOSE        "Close"
 ...
 * ask again: a portal that stayed up sends no owner change to recover on. */
#ifndef G_PASTE_PORTAL_BIND_RETRY_DELAY
#define G_PASTE_PORTAL_BIND_RETRY_DELAY 1000 /* ms */
#endif
#define G_PASTE_PORTAL_BIND_RETRIES     10
...
static gsize
accelerators_length (const GPasteKeybindingAccelerator *accels)
{
    gsize n = 0;
...
/* Copied strings and all: the caller owns the set it hands over for the length
 * of the call, and we hold on to it for as long as we hold the grabs -- which
 * is what lets us tell a set that has not changed from a new one. */
static GPasteKeybindingAccelerator *
accelerators_copy (const GPasteKeybindingAccelerator *accels)
{
    gsize n = accelerators_length (accels);
...
static void
accelerators_free (GPasteKeybindingAccelerator *accels)
{
    if (!accels)
        return;
 ...
 * of the value already stored included -- and the whole set is handed over on
 * every one of them. Asking the portal for the set it is already holding costs
 * the session whose permission the user has already given, and its dialog all
 * over again. */
static gboolean
accelerators_match (const GPasteKeybindingAccelerator *accels,
...
build_shortcuts_variant (GPasteGlobalShortcutClient *self)
{
    ...
    for (const GPasteKeybindingAccelerator *s = self->shortcuts; s->id; ++s)
    {
...
portal_request_data_new (GPasteGlobalShortcutClient *client,
                         GTask                      *task)
{
    ...
    if (!client->portal_owner)
        g_set_str (&client->portal_owner, data->owner);
    data->signal_id = 0;
    data->owner_watch = 0;
     ...
     * stopped answering. */
    client->requests = g_slist_prepend (client->requests, data);
...
{
    g_autoptr (GPasteGlobalShortcutClient) client = g_weak_ref_get (&data->client);
    ...
    if (client)
    {
        client->requests = g_slist_remove (client->requests, data);
    }
                     ...
                     const gchar     *token)
{
    ...
    return g_strdup_printf (G_PASTE_PORTAL_REQUEST_OBJECT_PATH "/%s/%s", sender, token);
}
...
static gchar *
portal_next_request_token (GPasteGlobalShortcutClient *self)
{
    return portal_token (++self->request_count);
}
...
static gchar *
portal_next_session_token (GPasteGlobalShortcutClient *self)
{
    return portal_token (++self->session_count);
}
                          ...
                          GDBusSignalCallback callback)
{
    ...
    data->signal_id = g_dbus_connection_signal_subscribe (
        data->connection, data->owner ? data->owner : G_PASTE_GLOBAL_SHORTCUT_BUS_NAME,
        G_PASTE_PORTAL_REQUEST_INTERFACE_NAME, G_PASTE_PORTAL_SIG_RESPONSE,
                   ...
                   gpointer      user_data)
{
    ...
    if (data->retries++ < G_PASTE_PORTAL_BIND_RETRIES)
    {
        g_debug ("GPasteGlobalShortcutClient: retrying portal session close: %s", error->message);
        ...
        guint source_id = g_timeout_add (G_PASTE_PORTAL_BIND_RETRY_DELAY,
                                         close_session_retry, g_steal_pointer (&data));
        g_source_set_name_by_id (source_id, "[GPaste] portal session close retry");
    }
    else
        g_warning ("GPasteGlobalShortcutClient: closing the portal session failed: %s", error->message);
}
...
close_session_retry (gpointer user_data)
{
    ...
    g_dbus_connection_call (data->connection, data->owner, data->handle,
                            G_PASTE_PORTAL_SESSION_INTERFACE_NAME, G_PASTE_PORTAL_CLOSE,
...
static void
clear_session (GPasteGlobalShortcutClient *self)
{
    g_clear_pointer (&self->session_handle, g_free);
    g_clear_pointer (&self->session_owner, g_free);
}
...
close_session (GPasteGlobalShortcutClient *self)
{
    ...
    g_autofree gchar *handle = g_steal_pointer (&self->session_handle);
    g_autofree gchar *owner = g_steal_pointer (&self->session_owner);
                    ...
                    gboolean            portal_is_gone)
{
    ...
    if (data->creates_session && !portal_is_gone && !data->responded)
    {
        ...
        {
            data->retire_source = g_timeout_add (G_PASTE_PORTAL_RETIRED_REQUEST_TIMEOUT,
...
static void
retire_requests (GPasteGlobalShortcutClient *self,
                 gboolean                    portal_is_gone)
{
    /* Retiring frees, and freeing unlinks: walk a copy of the list. */
    g_autoptr (GSList) requests = g_slist_copy (self->requests);
                      ...
                      gpointer         user_data)
{
    ...
    if (data->signal_id)
    {
        ...
        data->owner_signal_id = g_dbus_connection_signal_subscribe (
            data->connection, data->owner, G_PASTE_PORTAL_REQUEST_INTERFACE_NAME,
            G_PASTE_PORTAL_SIG_RESPONSE, data->request_path, NULL,
...
portal_request_watch_owner (_PortalRequestData *data)
{
    ...
    else
    {
        ...
        data->activation_watch = g_bus_watch_name_on_connection (data->connection,
            G_PASTE_GLOBAL_SHORTCUT_BUS_NAME, G_BUS_NAME_WATCHER_FLAGS_NONE,
...
static gboolean
has_live_request (GPasteGlobalShortcutClient *self)
{
    for (const GSList *r = self->requests; r; r = r->next)
    {
        const _PortalRequestData *data = r->data;

        if (data->generation == self->generation && !data->responded && !data->retired &&
...
static gboolean
portal_may_bind (GPasteGlobalShortcutClient *self)
{
    return self->shortcuts && self->shortcuts[0].id && !self->session_handle && !self->session_revoked &&
           !has_live_request (self);
}
                           ...
                           GPasteGlobalShortcutClient *client)
{
    ...
    return client->shortcuts && data->generation == client->generation &&
           !g_cancellable_is_cancelled (data->cancellable);
}
...
static void
supersede_session (GPasteGlobalShortcutClient *self)
{
    ...
    g_clear_handle_id (&self->retry_source, g_source_remove);
...
on_session_closed_signal (GDBusConnection *connection G_GNUC_UNUSED,
                          ...
                          gpointer         user_data)
{
    g_autoptr (GPasteGlobalShortcutClient) self = g_weak_ref_get (user_data);

    if (!self)
        return;


    if (!g_paste_str_equal (path, self->session_handle) ||
        !g_paste_str_equal (sender, self->session_owner))
        return;
...
{
    _PortalRequestData *data = user_data;
    g_autoptr (GPasteGlobalShortcutClient) self = g_weak_ref_get (&data->client);
    ...
    else if (response)
    {
        ...
        self->session_revoked = response == 1;
                  ...
                  GTask                      *task)
{
    ...
    g_autofree gchar *token = portal_next_request_token (self);
    ...
    g_dbus_connection_call (data->connection,
                            data->owner ? data->owner : G_PASTE_GLOBAL_SHORTCUT_BUS_NAME,
                            G_PASTE_GLOBAL_SHORTCUT_OBJECT_PATH,
                            G_PASTE_GLOBAL_SHORTCUT_INTERFACE_NAME,
                            G_PASTE_GLOBAL_SHORTCUT_BIND_SHORTCUTS,
                    ...
                    gpointer         user_data)
{
    ...
    g_set_str (&self->session_owner, sender);
                            ...
                            GTask                      *task)
{
    ...
    if (!self->session_closed_signal)
    {
        self->session_closed_signal = g_dbus_connection_signal_subscribe (
            data->connection, NULL, G_PASTE_PORTAL_SESSION_INTERFACE_NAME, "Closed",
    ...
    g_autofree gchar *session_token = portal_next_session_token (self);
    ...
    g_dbus_connection_call (data->connection,
                            ...
                            G_PASTE_GLOBAL_SHORTCUT_OBJECT_PATH,
                            G_PASTE_GLOBAL_SHORTCUT_INTERFACE_NAME,
                            G_PASTE_GLOBAL_SHORTCUT_CREATE_SESSION,
...
static void
on_bind_done (GObject      *source   G_GNUC_UNUSED,
                       GAsyncResult *result,
                       gpointer      user_data G_GNUC_UNUSED)
{
    ...
    if (!self->shortcuts || data->generation != self->generation)
        return;
    ...
    if (self->retries >= G_PASTE_PORTAL_BIND_RETRIES)
    {
        g_warning ("GPasteGlobalShortcutClient: binding the global shortcuts failed: %s", error->message);
        return;
    }

    g_debug ("GPasteGlobalShortcutClient: retrying global shortcut binding: %s", error->message);
     ...
     * it out of reach. */
    g_clear_handle_id (&self->retry_source, g_source_remove);
    self->retry_source = g_timeout_add_full (G_PRIORITY_DEFAULT, G_PASTE_PORTAL_BIND_RETRY_DELAY,
                                                     retry_grab_all, g_paste_weak_ref_new (self), g_paste_weak_ref_free);
    g_source_set_name_by_id (self->retry_source, "[GPaste] portal shortcut bind retry");
}
...
static void
start_grab_async (GPasteGlobalShortcutClient *self)
{
    ...
    g_autoptr (GTask) task = g_task_new (NULL, NULL, on_bind_done, NULL);
                                         ...
                                         const GPasteKeybindingAccelerator *accels)
{
    ...
    /* Safe to call on a disposed client: the shortcuts are then %NULL, and
     * dispose () has already closed the session that held them. */
    if (!self->shortcuts)
        return;
    ...
    if ((self->session_handle || has_live_request (self)) &&
        accelerators_match (self->shortcuts, accels))
        return;
    ...
    GPasteKeybindingAccelerator *shortcuts = accelerators_copy (accels);

    accelerators_free (self->shortcuts);
...
g_paste_global_shortcut_client_ungrab_all (GPasteGlobalShortcutClient *self)
{
     ...
     * none at all. */
    self->shortcuts = g_new0 (GPasteKeybindingAccelerator, 1);
    self->session_revoked = FALSE;
    self->retries = 0;
...
static void
portal_lost (GPasteGlobalShortcutClient *self)
{
...
static void
on_portal_name_owner_changed (GPasteGlobalShortcutClient *self,
                              ...
                              gpointer                    user_data G_GNUC_UNUSED)
{
    ...
    if (!owner || (self->portal_owner && !g_paste_str_equal (self->portal_owner, owner)))
        portal_lost (self);

    g_set_str (&self->portal_owner, owner);
...
g_paste_global_shortcut_client_dispose (GObject *object)
{
    ...
    g_autoptr (GSList) requests = g_steal_pointer (&self->requests);
    ...
    if (self->session_closed_signal)
    {
        g_dbus_connection_signal_unsubscribe (g_dbus_proxy_get_connection (G_DBUS_PROXY (self)),
                                              self->session_closed_signal);
        self->session_closed_signal = 0;
    }

    g_clear_pointer (&self->portal_owner, g_free);
    g_clear_pointer (&self->shortcuts, accelerators_free);
