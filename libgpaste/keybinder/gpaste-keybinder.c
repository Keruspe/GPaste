...
{
    if (k->action)
        g_paste_gnome_shell_client_ungrab_accelerator (k->shell_client, k->action, NULL, NULL);
...
static void
g_paste_keybinder_private_grab_all_internal (GPasteKeybinderPrivate *priv)
{
                      ...
                      gpointer      user_data)
{
    ...
    if (error)
    {
        ...
        g_paste_keybinder_private_grab_all_internal (priv);
    }
    else
    {
        guint index = 0;
