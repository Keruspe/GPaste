...
#define G_PASTE_SETTINGS_UI_WINDOW_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), G_PASTE_TYPE_SETTINGS_UI_WINDOW, GPasteSettingsUiWindowPrivate))

G_DEFINE_TYPE (GPasteSettingsUiWindow, g_paste_settings_ui_window, GTK_TYPE_WINDOW)

struct _GPasteSettingsUiWindowPrivate
{
    GPasteClient    *client;
...
g_paste_settings_ui_window_make_behaviour_panel (GPasteSettingsUiWindow *self)
{
    GPasteSettingsUiWindowPrivate *priv = self->priv;
...
static GPasteSettingsUiPanel *
g_paste_settings_ui_window_make_history_settings_panel (GPasteSettingsUiWindow *self)
{
    GPasteSettingsUiWindowPrivate *priv = self->priv;
    GPasteSettings *settings = priv->settings;
    GPasteSettingsUiPanel *panel = g_paste_settings_ui_panel_new ();
...
static GPasteSettingsUiPanel *
g_paste_settings_ui_window_make_keybindings_panel (GPasteSettingsUiWindow *self)
{
    GPasteSettingsUiWindowPrivate *priv = self->priv;
    GPasteSettings *settings = priv->settings;
    GPasteSettingsUiPanel *panel = g_paste_settings_ui_panel_new ();
...
static void
g_paste_settings_ui_window_refill_histories (GPasteSettingsUiWindow *self)
{
    GPasteSettingsUiWindowPrivate *priv = self->priv;
    GError *error = NULL;
...
backup_callback (const gchar *value,
                 gpointer     user_data)
{
    GPasteSettingsUiWindow *self = G_PASTE_SETTINGS_UI_WINDOW (user_data);
    GError *error = NULL;
    ...
    g_paste_settings_ui_window_refill_histories (self);
}
...
static GPasteSettingsUiPanel *
g_paste_settings_ui_window_make_histories_panel (GPasteSettingsUiWindow *self)
{
    GPasteSettingsUiWindowPrivate *priv = self->priv;
    GPasteSettings *settings = priv->settings;
    GPasteSettingsUiPanel *panel = g_paste_settings_ui_panel_new ();
...
static void
g_paste_settings_ui_window_fill (GPasteSettingsUiWindow *self)
{
    GPasteSettingsUiNotebook *notebook = g_paste_settings_ui_notebook_new ();

    g_paste_settings_ui_notebook_add_panel (notebook, _("General behaviour"), g_paste_settings_ui_window_make_behaviour_panel (self));
    g_paste_settings_ui_notebook_add_panel (notebook, _("History settings"), g_paste_settings_ui_window_make_history_settings_panel (self));
    g_paste_settings_ui_notebook_add_panel (notebook, _("Keyboard shortcuts"), g_paste_settings_ui_window_make_keybindings_panel (self));
    g_paste_settings_ui_notebook_add_panel (notebook, _("Histories"), g_paste_settings_ui_window_make_histories_panel (self));

    gtk_container_add (GTK_CONTAINER (self), GTK_WIDGET (notebook));
}

static void
g_paste_settings_ui_window_settings_changed (GSettings   *gsettings G_GNUC_UNUSED,
                                             const gchar *key,
                                             gpointer     user_data)
{
    GPasteSettingsUiWindowPrivate *priv = G_PASTE_SETTINGS_UI_WINDOW (user_data)->priv;
    GPasteSettings *settings = priv->settings;
...
static void
g_paste_settings_ui_window_dispose (GObject *object)
{
    GPasteSettingsUiWindowPrivate *priv = G_PASTE_SETTINGS_UI_WINDOW (object)->priv;
    GPasteSettings *settings = priv->settings;
    ...
    G_OBJECT_CLASS (g_paste_settings_ui_window_parent_class)->dispose (object);
}

static void
g_paste_settings_ui_window_finalize (GObject *object)
{
    gchar ***actions = G_PASTE_SETTINGS_UI_WINDOW (object)->priv->actions;

    for (guint i = 0; actions[i]; ++i)
        g_free ((gchar **)actions[i]);
    g_free ((gchar ***)actions);

    G_OBJECT_CLASS (g_paste_settings_ui_window_parent_class)->finalize (object);
}

static void
g_paste_settings_ui_window_class_init (GPasteSettingsUiWindowClass *klass G_GNUC_UNUSED)
{
    g_type_class_add_private (klass, sizeof (GPasteSettingsUiWindowPrivate));

    GObjectClass *object_class = G_OBJECT_CLASS (klass);

    object_class->dispose = g_paste_settings_ui_window_dispose;
    object_class->finalize = g_paste_settings_ui_window_finalize;
}

static void
g_paste_settings_ui_window_init (GPasteSettingsUiWindow *self)
{
    GPasteSettingsUiWindowPrivate *priv = self->priv = G_PASTE_SETTINGS_UI_WINDOW_GET_PRIVATE (self);

    priv->client = g_paste_client_new ();
    priv->settings = g_paste_settings_new ();
    priv->settings_signal = g_signal_connect (priv->settings,
                                              "changed",
                                              G_CALLBACK (g_paste_settings_ui_window_settings_changed),
                                              self);
...
g_paste_settings_ui_window_new (GtkApplication *application)
{
    ...
    g_paste_settings_ui_window_fill (self);
